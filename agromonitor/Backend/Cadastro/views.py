...
@csrf_exempt
@require_http_methods(["POST"])
def gerar_link_demo_api(request):
    """Gera um link de demonstração para completar cadastro sem enviar email real."""
    try:
        data = json.loads(request.body.decode('utf-8')) if request.body else {}
        usuario = (data.get('usuario') or 'demo').strip()
        email = (data.get('email') or 'demo@agromonitor.local').strip()
        ...
        if not creator:
            import uuid
            creator = Usuario.objects.create(
                usuario=f'demo_creator_{uuid.uuid4().hex[:6]}',
                email=f'demo_creator_{uuid.uuid4().hex[:6]}@agromonitor.local',
                email_hash=f'demo_creator_{uuid.uuid4().hex[:6]}_hash',
                senha_hash='demo_hash',
                role='owner',
            )

        if creator.role == 'owner' and role not in ['supervisor', 'employee']:
            return JsonResponse({'error': 'Usuários owner podem convidar apenas supervisor ou employee.'}, status=400)
        if creator.role in ['super_admin', 'admin'] and role != 'owner':
            return JsonResponse({'error': 'Usuários admin/super_admin podem convidar apenas owner.'}, status=400)
        ...
        url = f"{settings.FRONTEND_URL}/completar-cadastro/{convite.token}"
        return JsonResponse({
            'message': 'Link de demonstração gerado com sucesso.',
            'url': url,
...
def convidar_usuario_api(request):
    ...
    try:
        ...
        if UsuarioConvite.objects.filter(usuario=usuario).exists():
            return JsonResponse({'error': 'Já existe um convite pendente para este usuário.'}, status=409)

        if UsuarioConvite.objects.filter(email=email).exists():
            return JsonResponse({'error': 'Já existe um convite pendente para este email.'}, status=409)
...
def completar_cadastro_api(request):
    ...
    try:
        ...
        if novo_usuario.role == 'owner':
            try:
                call_command(
                    'provision_tenant',
                    tenant_name=novo_usuario.usuario,
                    slug=slugify(novo_usuario.usuario),
                    owner_id=novo_usuario.id,
                    owner_usuario=novo_usuario.usuario,
                    owner_email=novo_usuario.email,
                ...
                novo_usuario.delete()
                return JsonResponse({'error': f'Falha ao provisionar tenant: {exc}'}, status=500)
