...
class LocalidadeClima(models.Model):
    ...
    clima_tag = models.CharField('Tag de Clima', max_length=100, blank=True)
...
class NotaUsuario(models.Model):
    """Notas de usuário para consulta futura."""

    usuario = models.ForeignKey(
        'Cadastro.Usuario',
        on_delete=models.CASCADE,
        related_name='notas_clima'
    )
    texto = models.TextField('Texto da Anotação')
    criado_em = models.DateTimeField('Criado em', auto_now_add=True)
    atualizado_em = models.DateTimeField('Atualizado em', auto_now=True)
    ...
    def __str__(self):
        return f'Nota de {self.usuario.usuario} em {self.criado_em.isoformat()}'
