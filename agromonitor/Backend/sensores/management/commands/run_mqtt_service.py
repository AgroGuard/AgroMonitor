...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        mqtt_url = os.getenv('MQTT_BROKER_URL', options['broker'])
        ...
        mqtt_service = MQTTService(broker, port, debug, username=username, password=password)
        mqtt_service.start()
...
class MQTTService:
    ...
    def __init__(self, broker='localhost', port=1883, debug=False, username=None, password=None):
        self.broker = broker
        self.port = port
        self.debug = debug
