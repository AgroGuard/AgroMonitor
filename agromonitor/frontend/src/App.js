...
function App() {
  ...
  const role = isLoggedIn ? localStorage.getItem('userRole') : null;
  const noSidebarPaths = ['/', '/Login'];
  const mostrarSidebar = isLoggedIn && !noSidebarPaths.includes(location.pathname) && !location.pathname.startsWith('/recuperar');
